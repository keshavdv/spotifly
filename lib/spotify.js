...
Spotify.prototype._spircHandler = function(msg) {
  ...
  switch(msg.type) {
    ...
    case "kMessageTypeLoad":
      ...
      // this._spircBroadcast("kMessageTypeNotify");
      audioContext = msg.state
      this._playContext({index: msg.state.index, offset: msg.state.positionMs}, msg.state.track, msg);
      break;
    case "kMessageTypeSeek":
      if(msg.recipient.indexOf(this._ident) == -1) break;
      console.log("seeeking");
      console.log("state: " + JSON.stringify(this.state));
      this._playContext({index: this.state.index, offset: msg.position}, this.state.track, msg);
      break;
    default:
...
Spotify.prototype._playContext = function(args, list, msg) {
  console.log("stuck");
  var self = this;
  // if (self.stream != null && self.stream != undefined) self.stream.abort();
  // if (self.currentStream != null && self.currentStream != undefined) self.currentStream.end();
  console.log("stuck2");

  console.log("stream:" + self.stream);
  console.log("stream:" + JSON.stringify(list[args.index]));
  this.get(util.gid2uri("track", list[args.index].gid), function (err, track) {
    if (err) throw err;
    console.log('Playing: %s - %s', track.artist[0].name, track.name);
    self._notifyStateChange(msg)

    self.currentStream = track.play(args.offset).pipe(new lame.Decoder())
      .pipe(new Speaker())
  });
};

Spotify.prototype._notifyStateChange = function(msg) {
  this._deviceState.isActive = true;
  console.log("i got msg: " + JSON.stringify(msg));
  this.state = util.extend({}, this.state, msg.state);
  this.state.updated = (new Date()).getTime();
  console.log("i sett state: " + JSON.stringify(this.state));
