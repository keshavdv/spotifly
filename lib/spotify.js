...
var SpircFrame = schemas.build('spirc','Frame');
var SpircMessage = schemas.build('spirc','MessageType');
var SpircState = schemas.build('spirc','State');
var PubsubSubscription = schemas.build('pubsub','Subscription');
...
function Spotify () {
  ...
  // this._ident = (new Date()).getTime();
  this._ident = crypto.createHash('md5').update(os.hostname()).digest('binary');
  this._deviceState = {
                      "isActive": false,
                      "canPlay": true,
                      "volume": 65535,
...
Spotify.prototype._spircBroadcast = function(type) {
  var hm = 'hm://remote/user/keshavdv';
  this.sendProtobufRequest({
      ...
      payload:{
            ...
            "seqNr": (new Date()).getTime(),
            "type": type,
            "deviceState": this._deviceState,
            "state": this.state,
            "stateUpdateId": this.state.updated
        },
      responseSchema: SpircFrame,
      payloadSchema: SpircFrame
    }, function() {
      console.log('reply received');
    });
};

Spotify.prototype._spircHandler = function(msg) {
  debug('spirc msg: %j', msg);
  switch(msg.type) {
    case "kMessageTypeHello":
      this._spircBroadcast("kMessageTypeNotify")
      break;
    case "kMessageTypeLoad":
      if(msg.recipient.indexOf(this._ident) == -1) break;
      //TODO start playing
      this.state = { status: "kPlayStatusLoading" };
      this._deviceState.isActive = true;
      this.state.updated = (new Date()).getTime();
      this._spircBroadcast("kMessageTypeNotify");
      this._contextPlayer({index: msg.state.index, offset: msg.state.positionMs, contextUri: msg.state.contextUri}, msg.state.track, msg);
      break;
    case "kMessageTypeSeek":
      if(msg.recipient.indexOf(this._ident) == -1) break;
      console.log("seeeking");
      this._contextPlayer({index: this.state.index, offset: msg.position, contextUri: this.state.contextUri}, this.state.track, msg);
      break;
    case "kMessageTypePrev":
      if(msg.recipient.indexOf(this._ident) == -1) break;
      this._contextPlayer({index: this.state.index - 1, offset: msg.position, contextUri: this.state.contextUri}, this.state.track, msg);
      break;
    ...
    case "kMessageTypePause":
      this.state.status = "kPlayStatusPause";
      this.state.updated = (new Date()).getTime();
      this._spircBroadcast("kMessageTypeNotify");
      console.log("pausing");
    ...
    case "kMessageTypePlay":
      this.state.status = "kPlayStatusPlay";
      this.state.updated = (new Date()).getTime();
      this._spircBroadcast("kMessageTypeNotify");
    ...
    case "kMessageTypeNotify":
      if(msg.ident == this._ident) break;
      if(msg.deviceState.isActive) {
        console.log("found other active");
        this.currentTrack.pause();
        this._deviceState.isActive = false;
        this.state = {};
        self.state.updated = (new Date()).getTime();
        this._spircBroadcast("kMessageTypeNotify");
      }
      break;
    default:
...
Spotify.prototype._onmessagecommand = function (command, args) {
  if ('do_work' == command) {
    ...
    try {
      var message = this._parse(SpircFrame, new Buffer(args[2], 'base64'));  
      this._spircHandler(message);
    } catch(e) {
      // Not a SPIRC frame, so I don't know what to do       
      console.error(command, args);
    }
  }
};

Spotify.prototype._contextPlayer = function(args, playlist) {
  var self = this;
  if (self.currentTrack && self.currentTrack !== undefined) {
    this.currentTrack.stop();
  }

  this.get(util.gid2uri("track", playlist[args.index].gid), function (err, track) {
    if (err) throw err;
    console.log('Playing: %s - %s', track.artist[0].name, track.name);
    track.play(args.offset, function(err) {
      if(err) return;
      self._deviceState.isActive = true;
      self.state.contextUri = args.contextUri;
      self.state.index = args.index;
      self.state.track = playlist;
      self.state.positionMs = args.offset;
      self.state.positionMeasuredAt = (new Date()).getTime();
      self.state.status = "kPlayStatusPlay";
      self.state.updated = (new Date()).getTime();
      self._spircBroadcast("kMessageTypeNotify");
    })
  });
};
...
Spotify.prototype.sendPong = function(ping) {
  ...
  http.get('http://localhost/'+ encodedPing, function (res) {
    var resData = ''
...
Spotify.prototype.sendProtobufRequest = function(req, fn) {
  ...
  this.sendCommand('sp/hm_b64', args, function (err, res) {
    ...
    if(res.result.length == 1) {
      var data = parseData(header.contentType, new Buffer(res.result[0], 'base64'));
    } else {
...
Spotify.prototype.startConnect = function (deviceName) {
  var self = this;
  var hm = 'hm://remote/user/' + self.creds.username + '/r' + self.id + '/';

  self._deviceState.name = deviceName;
  console.log("oh i wan:" + self._deviceState.name);
  this.sendProtobufRequest({
    header: {
      method: 'SUB',
      uri: hm
    ...
    if(err) throw Error("Failed to start Connect server");
  });
};
/**
